...
build_table1_main <- function(all_s, tech_s, clin_s,
  ...
  tibble(
    Characteristic = c(
      ...
      "Silenced alarms",
      "  Silenced, n (%)",
      "  Time-to-silence, median [IQR], sec",
      "Alarm duration",
...
table1_main_JA <- build_table1_main(
  ...
  mutate(項目 = recode(項目,
    ...
    "  WARNING, n (%)"                       = "  WARNING, n (%)",
    "  CRISIS, n (%)"                        = "  CRISIS, n (%)",
    "Silenced alarms"                        = "消音行動",
    "  Silenced, n (%)"                      = "  消音あり, n (%)",
    "  Time-to-silence, median [IQR], sec"   = "  消音時間 中央値 [IQR]（秒）",
...
table1_clin_EN <- build_table1_clinical(
  ...
  row_dismissed = "Silenced, n (%)",
  row_ttd       = "Time-to-silence, median [IQR], sec",
